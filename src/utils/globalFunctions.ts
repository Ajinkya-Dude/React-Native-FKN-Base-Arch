...
export const formatDateAniversario = (value: any) => {
    if (!value) {
        return null;
    }
    const [_, day, month, year] = value && value.match(dateFormat);
    console.log("[_, day, month, year]",[_, day, month, year],"new Date(year, month - 1, day)",new Date(year, month - 1, day));
